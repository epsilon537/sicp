...
;Now define Q 1 to be the product of P 1 and P 2 , and Q 2 to be the product of P 1 and P 3 , and use greatest-common-divisor (Exercise 2.94) to compute the GCD of Q 1 and Q 2 . Note
;that the answer is not the same as P 1 . This example introduces noninteger operations into the computation, causing difficulties with the GCD algorithm.127 To understand what is
;happening, try tracing gcd-terms while computing the GCD or try performing the division by hand.
...
;(div q1 q2), long division by hand, shows that the coefficients from dividend and divisor don't divide cleanly (i.e. one is not an integer multiple of the other), so we end up with non-integer coefficients.
